...
export const AppProvider = ({ children }) => {
  ...
  const [currentPage, setCurrentPage] = useState(() => {
    // If we have a cached user, skip the select page and go to questions
    try {
      const cachedUser = localStorage.getItem(CURRENT_USER_CACHE_KEY);
      return (cachedUser === 'josh' || cachedUser === 'nini') ? 'questions' : 'select';
    } catch (error) {
      return 'select';
    }
  }); // 'select', 'questions', 'inbox', 'answers'
